...
public interface ProjectRepository extends JpaRepository<Project, Long> {
        ...
        // @Query(value="SELECT " +
        // "COALESCE(SUM(CASE WHEN LOWER(status) NOT LIKE 'rollout/solved' THEN 1 ELSE 0
        // END), 0) AS \"Not Done\", " +
        // "COALESCE(SUM(CASE WHEN LOWER(status) LIKE 'rollout/solved' AND lower(info1)
        // LIKE 'finished%overdue%' THEN 1 ELSE 0 END), 0) AS \"Overdue\", " +
        // "COALESCE(SUM(CASE WHEN LOWER(status) LIKE 'rollout/solved' AND lower(info1)
        // LIKE 'finished%on time%' THEN 1 ELSE 0 END), 0) AS \"On Time\", " +
        // "COALESCE(SUM(CASE WHEN LOWER(status) LIKE 'rollout/solved' AND lower(info1)
        // LIKE 'finished%ahead%' THEN 1 ELSE 0 END), 0) AS \"Ahead\", " +
        // "COALESCE(SUM(CASE WHEN LOWER(status) NOT LIKE 'rollout/solved' OR
        // (LOWER(status) LIKE 'rollout/solved' AND (LOWER(info1) LIKE
        // 'finished%overdue%') OR LOWER(info1) LIKE LOWER('Finished%On Time%') OR
        // LOWER(info1) LIKE LOWER('Finished%Ahead%')) THEN 1 ELSE 0 END), 0) as
        // \"Total\" " +
        // "FROM project WHERE type LIKE '%'||CASE WHEN :type = 'Insiden' THEN '%' ELSE
        // :type END||'%' AND category = :category AND month = :month AND period_id =
        // :period_id", nativeQuery = true)
        // LinkedMap<String,String> getCountProject(@Param("period_id") Long period_id,
        // @Param("month") String month,
        // @Param("category") String category,
        // @Param("type") String type);

        // @Query(value="SELECT a.total_project as \"Total Project\", a.total_selesai as
        // \"Total Selesai\", a.selesai_cepat as \"Selesai Cepat\", a.selesai_overdue as
        // \"Selesai Overdue\", CAST(SUM(CASE WHEN a.total_project = 0 OR
        // a.total_selesai = 0 THEN 0 ELSE (((CAST(a.total_selesai AS
        // FLOAT)/a.total_project)*1)+((CAST(a.selesai_cepat AS
        // FLOAT)/a.total_selesai)*:average_ahead)-((CAST(a.selesai_overdue AS
        // FLOAT)/a.total_selesai)*:average_overdue))*100 END) AS NUMERIC(4,0)) as
        // \"KPI\" " +
        // "FROM (SELECT " +
        // "COALESCE(SUM(CASE WHEN LOWER(status) NOT LIKE 'rollout/solved' OR
        // (LOWER(status) LIKE 'rollout/solved' AND (LOWER(info1) LIKE
        // 'finished%overdue%' OR LOWER(info1) LIKE 'finished%on time%' OR LOWER(info1)
        // LIKE 'finished%ahead%')) THEN 1 ELSE 0 END), 0) as total_project, " +
        // "COALESCE(SUM(CASE WHEN LOWER(status) LIKE 'rollout/solved' AND (LOWER(info1)
        // LIKE 'finished%overdue%' OR LOWER(info1) LIKE 'finished%on time%' OR
        // LOWER(info1) LIKE 'finished%ahead%') THEN 1 ELSE 0 END), 0) as total_selesai,
        // " +
        // "COALESCE(SUM(CASE WHEN LOWER(status) LIKE 'rollout/solved' AND lower(info1)
        // LIKE 'finished%ahead%' THEN 1 ELSE 0 END), 0) AS selesai_cepat, " +
        // "COALESCE(SUM(CASE WHEN LOWER(status) LIKE 'rollout/solved' AND lower(info1)
        // LIKE 'finished%overdue%' THEN 1 ELSE 0 END), 0) AS selesai_overdue " +
        // "FROM project " +
        // "WHERE month = :month AND period_id = :period_id AND LOWER(category) IN
        // :categories) as a " +
        // "GROUP BY a.total_project, a.total_selesai, a.selesai_cepat,
        // a.selesai_overdue", nativeQuery = true)
        ...
        // @Query(value="SELECT a.total_project as \"Total Project\", a.total_ontime as
        // \"Total Ontime\", a.selesai_cepat as \"Selesai Cepat\", a.selesai_overdue as
        // \"Selesai Overdue\", CAST(SUM(CASE WHEN a.total_project = 0 THEN 0 ELSE
        // (((CAST(a.total_ontime AS FLOAT)/a.total_project)*1)+((CAST(a.selesai_cepat
        // AS FLOAT)/a.total_project)*:average_ahead)+((CAST(a.selesai_overdue AS
        // FLOAT)/a.total_project)*:average_overdue))*100 END) AS NUMERIC(4,1)) as
        // \"KPI\" " +
        // "FROM " +
        // "(SELECT " +
        // "COALESCE(SUM(CASE WHEN LOWER(p.info1) LIKE 'finished%' AND LOWER(p.info1)
        // NOT LIKE 'finished%N/A%' THEN 1 ELSE 0 END), 0) as total_project, " +
        // "COALESCE(SUM(CASE WHEN LOWER(p.info1) LIKE 'finished%on time%' THEN 1 ELSE 0
        // END), 0) as total_ontime, " +
        // "COALESCE(SUM(CASE WHEN LOWER(p.info1) LIKE 'finished%ahead%' THEN 1 ELSE 0
        // END), 0) AS selesai_cepat, " +
        // "COALESCE(SUM(CASE WHEN LOWER(p.info1) LIKE 'finished%overdue%' THEN 1 ELSE 0
        // END), 0) AS selesai_overdue " +
        ...
        // ") as a " +
        // "GROUP BY a.total_project, a.total_ontime, a.selesai_cepat,
        // a.selesai_overdue", nativeQuery = true)
        // LinkedMap<String, String> getTotalProject(@Param("period_id") Long period_id,
        // @Param("rangeMonth") Iterable<String> rangeMonth,
        // @Param("categories") Iterable<String> categories,
